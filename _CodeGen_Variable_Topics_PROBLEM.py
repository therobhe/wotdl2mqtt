...
#--------------------ACTUATION-CALLBACKS----------------------------
# ToDO: generate this from the ontology, for every actuation that you find one of these blocks have to be put, in the blocks what is missing is the parameters: def invoke_implementation(function_name, params, kwargs, request, device), some are fixed like the device and some you get from the messages

#------------------LIGHT-ACTUATIONS----------------------------
# phil hue on
@subscribe('light/{id}/on')
...
# set thermostat temp
@subscribe('heating/{id}/setTemperature')
...
#-----------------SENSOR-CALLBACKS-------------------------------
#  ToDO: for the publish side you need to iterate over all the Measurements in the ontology and extract all the information about it, like the sensors,... and then do an infite loop that needs to periodically call the device implementations of each sensor (give me your data) and publish the value on the channel that is in the instance of the ontology to the MQTT broker, the interval has to be configurable (1 second for a start should be enough). You need to take care of loading the module (the device implementation) then look into it for the corresponding function, call the function and pack the return value that you get from it to a somewhat meaningful message (could be the value).
